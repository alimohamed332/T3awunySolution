...
namespace T3awuny.Application.Services
{
    public class AddressService : IAddressService
    {
        ...
        public async Task<AddressDetailsDto?> GetAddressByUserIdAsync(string userId)
        {
        ...
        public async Task<IReadOnlyList<AddressDetailsDto>> GetAllAddressesAsync()
        {
           var addresses = await _unitOfWork.Repository<Address>().GetAllAsync(); // no need to include => no need to use spec. no need to filter by userId since this is an admin endpoint
            return addresses.Select(address => _mapper.Map<AddressDetailsDto>(address)).ToList();
        }
        public async Task<AddressDetailsDto> AddAddressAsync(string userId, CreateAddressDto dto)
        {
            ...
            if (dto.IsDefault)
            {
                ...
                var existing = await _unitOfWork.Repository<Address>().GetByIdWithSpecAsync(addressSpecificationObj);
                if(existing is not null) 
                    existing.IsDefault = false;
            }
            else
                count = await _unitOfWork.Repository<Address>().GetCountAsync(new AddressSpecifications(a => a.UserId == userId));
